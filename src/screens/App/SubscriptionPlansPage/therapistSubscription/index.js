...
class SubscriptionPlansPage extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (data.length > 0) {
      const therapistPlans = data.filter(plan => plan.entity === 'therapist');
      if (therapistPlans.length > 0) {
        subscriptions = therapistPlans[0].plans.map(subscription => {
          return {
