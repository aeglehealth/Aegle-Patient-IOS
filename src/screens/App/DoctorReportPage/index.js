...
import {GET_REPORT} from '../../../QueryAndMutation';
...
export default class DoctorReportPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  async createPDF(description, doctorName, times, share) {
    ...
    if (granted) {
      const path = `${RNFetchBlob.fs.dirs.DownloadDir}/Notes_${date6()}.pdf`;
      try {
        const data = await RNFetchBlob.fs.writeFile(path, base64, 'base64');
        this.setState({loading: false});
        Alert.alert('Notes saved in the Download folder');
      } catch (error) {
        this.setState({loading: false});
            ...
            <View style={styles.container}>
              ...
              <ScrollView
                ...
                <View
                  ...
                  />
                  <Text style={styles.headerTitle}>Aegle Doctor's Report</Text>
                </View>
                <Text style={styles.sectionHeader}>DOCTOR REPORT</Text>
